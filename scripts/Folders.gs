 ...
 * @returns {GoogleAppsScript.Drive.Folder | null} New folder or null on error.
 */
 ...
 * WARNING: If multiple folders share the same name, the first one found will be used.
 * @param {string} parentFolderName Parent folder name.
 * @param {string} subfolderName Subfolder name.
 * @returns {GoogleAppsScript.Drive.Folder | null} New folder or null if parent not found or error.
 */
...
function createSubfolderById(parentFolderId, subfolderName) {
  ...
  try {
    // DriveApp.getFolderById throws an exception if the ID is invalid,
    // so we rely on the catch block for error handling.
